      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-white border border-border rounded-2xl p-6 mb-8 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <ImageUpload value={editing.coverImage ?? ""} originalValue={editing.coverImageOriginal ?? ""} heroValue={editing.coverImageHero ?? ""} cardValue={editing.coverImageCard ?? ""} multiContext onChange={(url, originalUrl) => setEditing((current) => current ? { ...current, coverImage: url, coverImageOriginal: originalUrl ?? "" } : current)} onChangeMulti={(urls) => setEditing((current) => current ? { ...current, ...urls } : current)} />
          </div>
