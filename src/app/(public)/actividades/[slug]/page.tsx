        ...
        <div className="relative max-w-[1400px] w-full mx-auto px-5 sm:px-6 lg:px-12 pb-8 sm:pb-12">
          <div className="flex gap-3 mb-4">
            {actividad.tags.map((tag) => (
              <span key={tag} className="text-[10px] font-medium tracking-[0.1em] uppercase text-white bg-rose/80 backdrop-blur-sm px-2.5 py-1 rounded-full">{tag}</span>
            ))}
          </div>
          ...
          {publishedMovimientos.length > 0 && (
            <div className="mt-12 pt-8 border-t border-border-light">
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-gold text-[12px] font-medium tracking-[0.15em] uppercase">Movimientos relacionados</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {publishedMovimientos.map((mov) => (
                  <Link key={mov.id} href={`/movimientos/${mov.slug}`} className="group block bg-white border border-border-light rounded-2xl p-5 hover:shadow-md transition-shadow">
                    <h3 className="font-serif text-lg text-primary font-normal group-hover:text-rose transition-colors">{mov.title}</h3>
