            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 sm:gap-8 -mx-5 sm:mx-0">
              {items.map((item) => (
                <Link
                  ...
                  className={`group block bg-white border hover:shadow-xl hover:shadow-black/5 hover:-translate-y-1 transition-all duration-500 rounded-2xl ${item.status === "draft" ? "border-gold border-dashed relative" : "border-border-light"}`}
                >
                  {item.status === "draft" && (
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="font-serif text-xl text-primary font-normal leading-tight group-hover:text-gold transition-colors">
                      {item.title}
                    </h2>
                    <p className="mt-3 text-text-secondary text-sm leading-relaxed font-light line-clamp-3">
                      {item.excerpt}
                    </p>
                    <div className="mt-4 flex items-center gap-2 text-gold text-[12px] font-medium tracking-[0.06em] uppercase">
                      Leer más
