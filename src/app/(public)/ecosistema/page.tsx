...
const empresas = Array.from({ length: 8 }, (_, i) => ({
  id: i + 1,
  name: `Empresa ${i + 1}`,
}));

const asociaciones = Array.from({ length: 6 }, (_, i) => ({
  id: i + 1,
  name: `Asociación ${i + 1}`,
}));

const instituciones = Array.from({ length: 4 }, (_, i) => ({
  id: i + 1,
  name: `Institución ${i + 1}`,
}));

const embajadores = Array.from({ length: 8 }, (_, i) => ({
  id: i + 1,
    ...
    <>
      ...
      <section className="py-12 sm:py-16 lg:py-24">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-6 lg:px-12">
          ...
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {empresas.map((e) => (
              <LogoPlaceholder key={e.id} name={e.name} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 lg:py-24 bg-surface-alt">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-6 lg:px-12">
          <ScrollReveal>
            ...
            <p className="mt-2 text-text-secondary text-sm leading-relaxed font-light max-w-3xl">
              Sus mensajes nos ayudan a potenciar el talento, evitar situaciones de discriminación, mejorar nuestra salud mental, bienestar, y por supuesto que los lugares de trabajo sean un reflejo de la sociedad.
            </p>
          </ScrollReveal>
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {asociaciones.map((a) => (
              <LogoPlaceholder key={a.id} name={a.name} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 lg:py-24">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-6 lg:px-12">
          ...
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {instituciones.map((i) => (
              <LogoPlaceholder key={i.id} name={i.name} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 lg:py-24 bg-surface-alt">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-6 lg:px-12">
          <ScrollReveal>
            ...
            <p className="mt-4 text-text-body text-base lg:text-lg leading-relaxed font-light max-w-3xl">
              Voces que inspiran desde su propia identidad y experiencia, llevando una forma más auténtica de entender el trabajo a las organizaciones. Profesionales que, desde sus ámbitos, impulsan el cambio y generan impacto.
            </p>
          </ScrollReveal>
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {embajadores.map((e) => (
